  ...
  pool.query(
    ...
    (err, userRows) => {
      ...
      if (userRows.length === 0) {
        return res.status(404).json({ error: "User not found — must be created at login" });
      }
          ...
          pool.query(
            ...
            (err, articleRows) => {
              ...
              if (articleRows.length === 0) {
                return res.status(404).json({ error: "Article not found after insert" });
              }
